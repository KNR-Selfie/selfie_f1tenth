...
public:
  ...
  void usb_read_buffer(int buf_size, uint32_t& timestamp,uint32_t& distance, float& velocity, float& quaternion_x, float& quaternion_y, float& quaternion_z, float& quaternion_w, float& ang_vel_x, float& ang_vel_y, float& ang_vel_z, float& lin_acc_x, float& lin_acc_y, float& lin_acc_z, uint8_t& taranis_3_pos, uint8_t& taranis_reset_gear, uint8_t& stm_reset);
